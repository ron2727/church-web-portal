...
<head>
    ...
    <title>Archived | Baptism Form</title>
</head>
<style>
   ul li{
    cursor: pointer;
                         ...
                         <div class="px-2 pt-3">
                              ...
                              <table id="tbBaptism" class="table table-striped table-bordered table-sm rounded-2" style="width:100%">
                                ...
                                <tbody id="table">
                                      ...
                                      <?php
                                       $type = $_GET['type'] ?? '';
                                       $sql = "SELECT * FROM baptism_form WHERE baptism_type = '$type' AND archived = 'yes'";
                                            ...
                                            <div class="dropdown">
                                               <button class="btnDelete btn btn-danger badge rounded-pill" data-bs-toggle="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                   <li><a class="dropdown-item text-success" href="print_form.php?type=<?php echo $row['baptism_type']?>&service=baptism&form_id=<?php echo $row['form_id']?>" target="_blank"><i class="bi bi-clipboard2-fill"></i> Form</a></li>
                                                   <?php if($row['status'] == 'Completed'):?>
                                                      <li><a class="dropdown-item text-danger" href="baptism_certificate.php?type=<?php echo $row['baptism_type']?>&form_id=<?php echo $row['form_id']?>" target="_blank"><i class="bi bi-file-earmark-pdf-fill"></i> Certificate</a></li>
                                                   <?php endif;?>
                                                </ul>
                                             </div>
                                            <?php if($row['status'] !== 'Completed'):?>
                                              <a class="text-danger" href="retrieve_service.php?service=baptism&type=<?php echo $row['baptism_type']?>&form_id=<?php echo $row['form_id']?>" target="_blank">
                                                <button type="button" class="btnDelete btn btn-danger badge rounded-pill"><i class="bi bi-recycle"></i> Retrieve</button>
                                              </a>
                                            <?php endif;?>
                                          </td>                              
                                      </tr>
     ...
     <script type="text/javascript">
          $(document).ready(function(){
            ...
            $('.archived-menus').removeClass('collapse');
              ...
              $('.bap-menus1').removeClass('collapse')
              if (type === 'child') {
                $('.child1-link').addClass('rounded-3 bg-primary')
              }
              if (type === 'youth') {
                $('.youth1-link').addClass('rounded-3 bg-primary')
              }
                 if (action == 'retrieve') {
                toastr.success('Retrieve Successfully', 'Retrieve Baptism Form');
              }
              $('#tbBaptism').DataTable({
                    autoWidth: false,
                    columnDefs: [
